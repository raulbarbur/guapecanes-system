...
import CustomerSaleRow from "@/components/CustomerSaleRow" // 👈 Crearemos este componente pequeño abajo para la interactividad
...
export default async function CustomerDetailPage({ params }: Props) {
  ...
  const customer = await prisma.customer.findUnique({
    ...
    include: {
      sales: {
        where: { status: 'COMPLETED' }, // Solo ventas válidas
  ...
  // Calcular Total Histórico Gastado (LTV)
  const lifetimeValue = customer.sales
    .reduce((sum, s) => sum + Number(s.total), 0)
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* COLUMNA DERECHA: HISTORIAL DE VENTAS Y DEUDA */}
