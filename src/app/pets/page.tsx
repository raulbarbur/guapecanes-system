...
import { SubmitButton } from "@/components/ui/SubmitButton" // << FE-02: Importar
...
export default async function PetsPage({ searchParams }: Props) {
  ...
  // ✅ FIX: Agregamos 'use server' para que Next.js reconozca esto como una Server Action válida
  async function createPetAction(formData: FormData) {
    'use server' // << ESTO FALTABA
          ...
          <div className="bg-card p-6 rounded-3xl shadow-sm border border-border sticky top-6">
            ...
            <form action={createPetAction} className="space-y-4">
              <div>
                ...
                <input name="name" type="text" required placeholder="Ej: Bobby" className={inputClass} autoFocus/>
              </div>
              <div>
              ...
              {/* FE-02: Usamos SubmitButton para evitar doble clic */}
              <div className="mt-4">
