...
export default function SettlementForm({ ownerId, items }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // R-08: Reemplazar confirm por un modal (si existiera) o mantenerlo como acción crítica
                            ...
                            {items.map(row => {
                                ...
                                return (
                                    <tr key={row.id} className={cn("transition", isSelected ? "bg-primary/5" : "hover:bg-muted/30 opacity-60 hover:opacity-100")}>
                                        ...
                                        <td className="p-4"><p className="font-bold text-foreground flex items-center gap-2">{row.type === 'SALE' ? '🛒' : '↩️'} {row.description}</p><p className="text-xs text-muted-foreground">{row.date.toLocaleDateString()}</p></td>
